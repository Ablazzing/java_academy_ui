...
const AdminModuleUpdatePage = () => {
  ...
  const form = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const response = await appApi().modules.updateModule(params)
      if(response) {
        NotificationManager.success(messages.module.success.update)
      } else {
        NotificationManager.error(messages.module.errors.update)
      }
      setLoading(false)
    }
  })
  ...
  const loadPageData = async () => {
    ...
    if(response && response[0]) {
      ...
      form.setFieldValue('name', response[0].name)
      form.setFieldValue('russianName', response[0].russianName)
      form.setFieldValue('description', response[0].description)
      form.setFieldValue('order', isNumber(response[0].order) ? parseInt(response[0].order) : '')
      form.setFieldValue('positionTop', isNumber(response[0].position.top) ? parseInt(response[0].position.top) : '')
      form.setFieldValue('positionLeft', isNumber(response[0].position.left) ? parseInt(response[0].position.left) : '')
      form.setFieldValue('positionWidth', isNumber(response[0].position.width) ? parseInt(response[0].position.width) : '')
      form.setFieldValue('isFree', response[0].isFree ? '1' : '0')
      form.setFieldValue('price', isNumber(response[0].price) ? parseInt(response[0].price) : '')
    } else {
      setError(true)
    }
    closeLoader()
  ...
  return (
    <AppLayout title={ error ? 'Модуль не найден' : `Редактирование модуля ${ module.russianName }` }>
      <div className="wrap container">
        <Breadcrumbs />
        <div className="pagetitle admin">
          <span>Редактирование модуля { module.russianName }</span>
        </div>
