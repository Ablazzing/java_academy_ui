...
const TestPage = () => {
  ...
  const loadPageData = async () => {
    ...
    if(tempModule) {
      const test = await WayRepository.getTest({
        slug: router.query.module
      })
      if(test) {
  ...
  return (
    <UserwayLayout>
      <Head>
        <title>{ error ? 'Тест не найден' : `Модуль ${ module.russianName }` }</title>
      </Head>
        ...
        <Topbar data={{ label: 'Структуры данных +Stream api', link: '/' }}/>
        {
          ...
          <>
            ...
            <div className="swiperbox">
              <Swiper navigation={ navigationBtns } modules={ swiperModules } autoHeight={ true } ref={ refSwiper } >
                <SwiperSlide>
