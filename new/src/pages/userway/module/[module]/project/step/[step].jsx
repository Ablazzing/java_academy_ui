...
const StepPage = () => {
  ...
  const modules = useSelector(state => state.way.modules)
  ...
  const loadPageData = async () => {
    ...
    if(tempModule && (!tempModule.stepsData || !tempModule.stepsData[router.query.step])) {
      const step = await WayRepository.getStep({
        slug: router.query.module,
  ...
  return (
    <UserwayLayout>
      <Head>
        <title>{ error ? 'Этап не найден' : module?.project?.projectInfo?.russianName }</title>
      </Head>
